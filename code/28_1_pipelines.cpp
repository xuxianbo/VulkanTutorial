...
private:
    ...
    // 屏幕右边
    VkPipeline graphicsPipelineRight;
    ...
    void cleanup() {
        ...
        vkDestroyPipeline(device, graphicsPipelineRight, nullptr);
    ...
    void createGraphicsPipeline() {
        ...
        rasterizer.polygonMode = VK_POLYGON_MODE_FILL;          // 正常默认绘制
        // rasterizer.polygonMode = VK_POLYGON_MODE_POINT;      // 绘制点模式
        // rasterizer.polygonMode = VK_POLYGON_MODE_LINE;       // 绘制线模式
        ...
        {
            throw std::runtime_error("failed to create graphics PipelineCache!");
        }
        ...
        if (vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineInfo, nullptr, &graphicsPipelineRight)!= VK_SUCCESS) 
        {
            throw std::runtime_error("failed to create graphics pipeline right!");
        }
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
            ...
            viewport.height = (float) swapChainExtent.height/2.;
            vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
            vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelineRight);


            // 测试常量  pushConst传入
            // pushConstBlock.color = glm::vec3(0.1f, 0.0f, 0.0f);
            // vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT,0, sizeof(pushConstBlock), &pushConstBlock);
    ...
    void drawFrame() {
        ...
        // 因为如果不同步  继续走 走到下一次recordCommandBuffer的的地方  当前帧的commandBuffers会被释放
        if (vkQueueSubmit(graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) != VK_SUCCESS) {
            throw std::runtime_error("failed to submit draw command buffer!");
        }

        // Semaphores信号量  是在命令队列之间同步
        ...
        presentInfo.pImageIndices = &imageIndex;    // 写入那张图
